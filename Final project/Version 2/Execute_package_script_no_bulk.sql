create or replace procedure  job_frmwrk.g_sp_execute_cust_xport_nobulk 

/*********************************************************************************************************
**    OBJECT NAME:  g_sp_execute_cust_xport_nobulk
**
...
begin
    ...
    dbms_output.put_line ('Inserting staging tracking id for account export job with no bulk fetch. Time '||to_char(sysdate, 'MM/DD/YYYY HH24:MI:SS'));
    ...
    dbms_output.put_line ('Starting account info stage job with no bulk fetch. Batch tracking id : '|| l_v_stg_job_trckg_id ||' Time '||to_char(sysdate, 'MM/DD/YYYY HH24:MI:SS'));
    ...
    dbms_output.put_line ('Starting account info export job with no bulk fetch. Batch tracking id : '|| l_v_xport_job_trckg_id ||' Time '||to_char(sysdate, 'MM/DD/YYYY HH24:MI:SS'));
    job_frmwrk.pkg_acct_info_export_no_bulk.g_sp_acct_info_xport(l_v_xport_job_trckg_id);  
    
    dbms_output.put_line ('Finished account info export inserts and writes without using bulk collect. Time '||to_char(sysdate, 'MM/DD/YYYY HH24:MI:SS'));  
